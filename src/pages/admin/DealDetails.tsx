...
import { useGetAdminDealQuery, useGetDealStatsQuery, useGetPublicSettingsQuery } from '@/hooks/useApi';
...
export default function DealDetails() {
  ...
  const { data: dealData, isLoading: dealLoading } = useGetAdminDealQuery(id!);
          ...
          <CardContent className="space-y-4">
            {deal.applicable_products?.length > 0 && (
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {deal.products_details ? (
                    deal.products_details.map((p: any) => (
                      <Badge key={p.id} variant="secondary">
                        {p.name} <span className="text-[10px] ml-1 opacity-70">({p.sku})</span>
                      </Badge>
                    ))
                  ) : (
                    deal.applicable_products.map((id: any) => (
            ...
            {deal.applicable_categories?.length > 0 && (
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {deal.categories_details ? (
                    deal.categories_details.map((c: any) => (
                      <Badge key={c.id} variant="secondary">{c.name}</Badge>
                    ))
                  ) : (
                    deal.applicable_categories.map((id: any) => (
